...
SEXP strings_dot_min_before = NULL;
SEXP strings_dot_min_after = NULL;
...
void slider_initialize_utils(SEXP ns) {
  ...
  strings_dot_min_before = Rf_allocVector(STRSXP, 1);
  R_PreserveObject(strings_dot_min_before);
  SET_STRING_ELT(strings_dot_min_before, 0, Rf_mkChar(".min_before"));

  strings_dot_min_after = Rf_allocVector(STRSXP, 1);
  R_PreserveObject(strings_dot_min_after);
  SET_STRING_ELT(strings_dot_min_after, 0, Rf_mkChar(".min_after"));
